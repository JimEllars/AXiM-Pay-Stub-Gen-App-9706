...
const InputField = ({ label, value, onChange, type = "text", placeholder }) => (
  <div className="flex flex-col gap-1.5 mb-4">
    ...
    <input 
      ...
      className="bg-black/50 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-axim-teal focus:ring-1 focus:ring-axim-teal transition-all font-mono"
    />
  </div>
);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-glass border border-white/10 p-6 rounded-xl backdrop-blur-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <InputField label="Company Name" value={employerDetails.name} onChange={(v) => updateEmployer('name', v)} placeholder="Acme Corp" />
      ...
      <div className="bg-glass border border-white/10 p-6 rounded-xl backdrop-blur-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <InputField label="Full Name" value={employeeDetails.name} onChange={(v) => updateEmployee('name', v)} placeholder="John Doe" />
          <div className="flex flex-col gap-1.5 mb-4">
      ...
      <div className="bg-glass border border-white/10 p-6 rounded-xl backdrop-blur-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <InputField label="Start Date" type="date" value={payPeriod.startDate} onChange={(v) => updatePayPeriod('startDate', v)} />
          <InputField label="End Date" type="date" value={payPeriod.endDate} onChange={(v) => updatePayPeriod('endDate', v)} />
        </div>
      </div>
    </div>
  );
